...
TEST_SUITE("bust.type_unifier") {
  ...
  TEST_CASE("unresolved type variable finds back to itself") {
    ...
    CHECK(std::holds_alternative<hir::TypeVariable>(type_arena.get(result)));
  }
  ...
  TEST_CASE("unify type variable with concrete type resolves it") {
    ...
    unifier.unify(t0, type_arena.m_i64);

    auto resolved = unifier.find(t0);
    REQUIRE(std::holds_alternative<hir::PrimitiveTypeValue>(
        type_arena.get(resolved)));
    CHECK(resolved == type_arena.m_i64);
  }
  ...
  TEST_CASE("unify concrete type with type variable (reversed arg order)") {
    ...
    unifier.unify(type_arena.m_bool, t0);

    auto resolved = unifier.find(t0);
    REQUIRE(std::holds_alternative<hir::PrimitiveTypeValue>(
        type_arena.get(resolved)));
    CHECK(resolved == type_arena.m_bool);
  }
  ...
  TEST_CASE("unify same type variable with same concrete type twice is ok") {
    ...
    unifier.unify(t0, type_arena.m_i64);
    CHECK_NOTHROW(unifier.unify(t0, type_arena.m_i64));
  }
  ...
  TEST_CASE("unify type variable with conflicting concrete types throws") {
    ...
    unifier.unify(t0, type_arena.m_i64);
    CHECK_THROWS(unifier.unify(t0, type_arena.m_bool));
  }
  ...
  TEST_CASE("unify two unresolved type variables links them") {
    ...
    REQUIRE(std::holds_alternative<hir::TypeVariable>(type_arena.get(r0)));
    REQUIRE(std::holds_alternative<hir::TypeVariable>(type_arena.get(r1)));
    CHECK(std::get<hir::TypeVariable>(type_arena.get(r0)).m_id ==
          std::get<hir::TypeVariable>(type_arena.get(r1)).m_id);
  }
  ...
  TEST_CASE("unify two type variables then resolve one resolves both") {
    ...
    auto r1 = unifier.find(t1);
    REQUIRE(
        std::holds_alternative<hir::PrimitiveTypeValue>(type_arena.get(r0)));
    REQUIRE(
        std::holds_alternative<hir::PrimitiveTypeValue>(type_arena.get(r1)));
    CHECK(r0 == type_arena.m_i64);
    CHECK(r1 == type_arena.m_i64);
  }
  ...
  TEST_CASE("unify resolved variable with unresolved propagates type") {
    ...
    unifier.unify(t0, type_arena.m_bool);
    unifier.unify(t0, t1);
  ...
  TEST_CASE("unify two variables both resolved to same type is ok") {
    ...
    unifier.unify(t0, type_arena.m_i64);
    unifier.unify(t1, type_arena.m_i64);
  ...
  TEST_CASE("unify two variables resolved to different types throws") {
    ...
    unifier.unify(t0, type_arena.m_i64);
    unifier.unify(t1, type_arena.m_bool);
  ...
  TEST_CASE("chain of three variables resolves transitively") {
    ...
    unifier.unify(t2, type_arena.m_i64);
    ...
    REQUIRE(
        std::holds_alternative<hir::PrimitiveTypeValue>(type_arena.get(r1)));
    REQUIRE(
        std::holds_alternative<hir::PrimitiveTypeValue>(type_arena.get(r2)));
    CHECK(r0 == type_arena.m_i64);
    CHECK(r1 == type_arena.m_i64);
    CHECK(r2 == type_arena.m_i64);
  }
  ...
  TEST_CASE("independent type variables do not affect each other") {
    ...
    auto r1 = unifier.find(t1);
    CHECK(std::holds_alternative<hir::TypeVariable>(type_arena.get(r1)));
  }
...
TEST_SUITE("bust.free_type_variable_collector") {
  ...
  TEST_CASE("collects type variable from bare TV") {
    ...
    hir::FreeTypeVariableCollector collector{type_arena};
  ...
  TEST_CASE("collects TVs from inside FunctionType") {
    ...
    params.emplace_back(type_arena.intern(t0));
    hir::TypeKind fn_type =
        hir::FunctionType{std::move(params), type_arena.intern(t1)};
  ...
  TEST_CASE("collects only TVs not concrete parts of fn type") {
    ...
    std::vector<hir::TypeId> params;
    params.emplace_back(type_arena.m_i64);
    hir::TypeKind fn_type =
        hir::FunctionType{std::move(params), type_arena.intern(t1)};
...
TEST_SUITE("bust.type_unifier.function_types") {
  ...
  TEST_CASE("unify two identical concrete function types") {
    ...
    auto make_fn = [&]() {
      std::vector<hir::TypeId> params;
      params.emplace_back(type_arena.m_i64);
      return hir::FunctionType{std::move(params), type_arena.m_bool};
    };

    hir::TypeKind fn_a = make_fn();
    hir::TypeKind fn_b = make_fn();
    CHECK_NOTHROW(
        unifier.unify(type_arena.intern(fn_a), type_arena.intern(fn_b)));
  }
  ...
  TEST_CASE("unify function types with different return types throws") {
    ...
    params_a.emplace_back(type_arena.m_i64);
    hir::TypeKind fn_a =
        hir::FunctionType{std::move(params_a), type_arena.m_bool};

    std::vector<hir::TypeId> params_b;
    params_b.emplace_back(type_arena.m_i64);
    hir::TypeKind fn_b =
        hir::FunctionType{std::move(params_b), type_arena.m_i64};
  ...
  TEST_CASE("unify function types with different arity throws") {
    ...
    params_a.emplace_back(type_arena.m_i64);
    hir::TypeKind fn_a =
        hir::FunctionType{std::move(params_a), type_arena.m_i64};
  ...
  TEST_CASE("unify function type with type variable resolves it") {
    ...
    params.emplace_back(type_arena.m_i64);
    hir::TypeKind fn_type =
        hir::FunctionType{std::move(params), type_arena.m_bool};

    unifier.unify(type_arena.intern(t0), type_arena.intern(fn_type));

    auto resolved = unifier.find(t0);
    REQUIRE(
        std::holds_alternative<hir::FunctionType>(type_arena.get(resolved)));
  }
  ...
  TEST_CASE("unify function type containing type variables resolves params") {
    ...
    params_b.emplace_back(type_arena.m_i64);
    hir::TypeKind fn_b =
        hir::FunctionType{std::move(params_b), type_arena.m_bool};
  ...
  TEST_CASE("unify two concrete mismatched types throws") {
    ...
    CHECK_THROWS(unifier.unify(type_arena.m_i64, type_arena.m_bool));
  }
  ...
  TEST_CASE("unify two identical concrete types is ok") {
    ...
    CHECK_NOTHROW(
        unifier.unify(type_arena.intern(i64_a), type_arena.intern(i64_b)));
  }
  ...
  TEST_CASE("never type unifies with anything") {
    ...
    CHECK_NOTHROW(unifier.unify(type_arena.intern(never), type_arena.m_i64));
    CHECK_NOTHROW(unifier.unify(type_arena.m_bool, type_arena.intern(never)));
    CHECK_NOTHROW(
        unifier.unify(type_arena.intern(never), type_arena.intern(never)));
  }
}
...
TEST_SUITE("bust.type_unifier.constraints") {
  ...
  TEST_CASE("constrain then resolve with compatible type succeeds") {
    ...
    CHECK_NOTHROW(unifier.unify(type_arena.intern(t0), type_arena.m_i64));
  }
  ...
  TEST_CASE("constrain then resolve with i8 succeeds") {
    ...
    auto t0 = unifier.new_type_var();
    unifier.constrain(t0, PrimitiveTypeClass::NUMERIC);
    CHECK_NOTHROW(unifier.unify(type_arena.intern(t0), type_arena.m_i8));
  }
  ...
  TEST_CASE("constrain then resolve with i32 succeeds") {
    ...
    auto t0 = unifier.new_type_var();
    unifier.constrain(t0, PrimitiveTypeClass::NUMERIC);
    CHECK_NOTHROW(unifier.unify(type_arena.intern(t0), type_arena.m_i32));
  }
  ...
  TEST_CASE("constrain NUMERIC then resolve with bool throws") {
    ...
    auto t0 = unifier.new_type_var();
    unifier.constrain(t0, PrimitiveTypeClass::NUMERIC);
    CHECK_THROWS(unifier.unify(type_arena.intern(t0), type_arena.m_bool));
  }
  ...
  TEST_CASE("constrain NUMERIC then resolve with char throws") {
    ...
    auto t0 = unifier.new_type_var();
    unifier.constrain(t0, PrimitiveTypeClass::NUMERIC);
    CHECK_THROWS(unifier.unify(type_arena.intern(t0), type_arena.m_char));
  }
  ...
  TEST_CASE("constrain NUMERIC then resolve with unit throws") {
    ...
    auto t0 = unifier.new_type_var();
    unifier.constrain(t0, PrimitiveTypeClass::NUMERIC);
    CHECK_THROWS(unifier.unify(type_arena.intern(t0), type_arena.m_unit));
  }
  ...
  TEST_CASE("constrain BOOL then resolve with bool succeeds") {
    ...
    CHECK_NOTHROW(unifier.unify(type_arena.intern(t0), type_arena.m_bool));
  }
  ...
  TEST_CASE("constrain BOOL then resolve with i64 throws") {
    ...
    auto t0 = unifier.new_type_var();
    unifier.constrain(t0, PrimitiveTypeClass::BOOL);
    CHECK_THROWS(unifier.unify(type_arena.intern(t0), type_arena.m_i64));
  }
  ...
  TEST_CASE("constrain COMPARABLE then resolve with char succeeds") {
    ...
    auto t0 = unifier.new_type_var();
    unifier.constrain(t0, PrimitiveTypeClass::COMPARABLE);
    CHECK_NOTHROW(unifier.unify(type_arena.intern(t0), type_arena.m_char));
  }
  ...
  TEST_CASE("resolve then constrain with compatible class succeeds") {
    ...
    hir::TypeUnifier unifier{type_arena};
    auto t0 = unifier.new_type_var();
    unifier.unify(type_arena.intern(t0), type_arena.m_i64);
    CHECK_NOTHROW(unifier.constrain(t0, PrimitiveTypeClass::NUMERIC));
  }
  ...
  TEST_CASE("resolve then constrain with incompatible class throws") {
    ...
    hir::TypeUnifier unifier{type_arena};
    auto t0 = unifier.new_type_var();
    unifier.unify(type_arena.intern(t0), type_arena.m_bool);
  ...
  TEST_CASE("resolve to char then constrain NUMERIC throws") {
    ...
    hir::TypeUnifier unifier{type_arena};
    auto t0 = unifier.new_type_var();
    unifier.unify(type_arena.intern(t0), type_arena.m_char);
    CHECK_THROWS(unifier.constrain(t0, PrimitiveTypeClass::NUMERIC));
  }
  ...
  TEST_CASE(
    ...
    CHECK_THROWS(unifier.unify(type_arena.intern(t1), type_arena.m_bool));
    CHECK_NOTHROW(unifier.unify(type_arena.intern(t1), type_arena.m_i64));
  }
  ...
  TEST_CASE("two NUMERIC vars unified then resolved") {
    ...
    REQUIRE(
        std::holds_alternative<hir::PrimitiveTypeValue>(type_arena.get(r1)));
    CHECK(r0 == type_arena.m_i8);
    CHECK(r1 == type_arena.m_i8);
  }
  ...
  TEST_CASE("constrained var unified with concrete-resolved var succeeds") {
    ...
    auto t1 = unifier.new_type_var();
    unifier.constrain(t0, PrimitiveTypeClass::NUMERIC);
    unifier.unify(type_arena.intern(t1), type_arena.m_i64);
    CHECK_NOTHROW(unifier.unify(t0, t1));
  }
  ...
  TEST_CASE("constrained var unified with incompatible concrete var throws") {
    ...
    auto t1 = unifier.new_type_var();
    unifier.constrain(t0, PrimitiveTypeClass::NUMERIC);
    unifier.unify(type_arena.intern(t1), type_arena.m_bool);
    CHECK_THROWS(unifier.unify(t0, t1));
  }
  ...
  TEST_CASE("concrete-resolved var unified with constrained var succeeds") {
    ...
    auto t0 = unifier.new_type_var();
    auto t1 = unifier.new_type_var();
    unifier.unify(type_arena.intern(t0), type_arena.m_i32);
    unifier.constrain(t1, PrimitiveTypeClass::NUMERIC);
    CHECK_NOTHROW(unifier.unify(t0, t1));
  }
  ...
  TEST_CASE("constraint propagates through chain of unified vars") {
    ...
    CHECK_THROWS(unifier.unify(type_arena.intern(t2), type_arena.m_bool));
    CHECK_NOTHROW(unifier.unify(type_arena.intern(t2), type_arena.m_i64));
  }
