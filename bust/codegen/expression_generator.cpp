...
Handle ExpressionGenerator::operator()(const hir::Identifier &identifier) {
  ...
  m_ctx.block().add_instruction(LoadInstruction{
      ...
      .m_type = to_llvm_type(m_ctx.type_arena().get(identifier.m_type))});
...
Handle ExpressionGenerator::operator()(
  ...
  const auto if_type_is_unit = if_return_type_id == m_ctx.type_arena().m_unit;
  ...
  function.add_alloca_instruction(AllocaInstruction{
      ...
      .m_type = to_llvm_type(m_ctx.type_arena().get(if_return_type_id))});
...
Handle ExpressionGenerator::operator()(
  ...
  auto function_return_type_id =
      std::get<hir::FunctionType>(
          m_ctx.type_arena().get(call_expression->m_callee.m_type))
          .m_return_type;

  if (function_return_type_id == m_ctx.type_arena().m_unit) {
  ...
  m_ctx.block().add_instruction(CallInstruction{
      ...
      .m_arguments = std::move(arguments),
      .m_return_type =
          to_llvm_type(m_ctx.type_arena().get(function_return_type_id))});
...
Handle ExpressionGenerator::generate_integer_compare_instruction(
  ...
  auto compare_condition = to_llvm_compare_condition(
      ...
      m_ctx.type_arena().get(binary_expression->m_lhs.m_type));
...
Handle ExpressionGenerator::operator()(
  ...
  m_ctx.block().add_instruction(UnaryInstruction{
      ...
      .m_type = to_llvm_type(
          m_ctx.type_arena().get(unary_expression->m_expression.m_type)),
  });
...
Handle ExpressionGenerator::operator()(
  ...
  const auto &from = to_llvm_type(
      m_ctx.type_arena().get(cast_expression->m_expression.m_type));
  const auto &to =
      to_llvm_type(m_ctx.type_arena().get(cast_expression->m_new_type));
